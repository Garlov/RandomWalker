...
int Cube::draw(GLfloat* g_vertex_buffer_data, GLfloat* g_color_buffer_data, int index) {
	int vertexLength = (sizeof(cubeVertex) / sizeof(*cubeVertex));
	for (int i = 0; i < vertexLength; i++) {
		g_vertex_buffer_data[index + i] = cubeVertex[i];
	}
	int colorLength = (sizeof(cubeColor) / sizeof(*cubeColor));
	for (int i = 0; i < colorLength; i++) {
		g_color_buffer_data[index + i] = cubeColor[i];
	}
